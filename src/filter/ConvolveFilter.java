...
/**
 * A filter which applies a convolution kernel to an filter.
 *
...
public class ConvolveFilter extends AbstractBufferedImageOp {
     ...
     */
    static int CLAMP_EDGES = 1;

    /**
     * Wrap pixels off the edge to the opposite edge.
     */
    static int WRAP_EDGES = 2;
    ...
    /**
     * What do do at the filter edges.
     */
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        ...
        if (premultiplyAlpha)
            ImageMath.premultiply(inPixels, inPixels.length);
        convolve(kernel, inPixels, outPixels, width, height, alpha, edgeAction);
        if (premultiplyAlpha)
            ImageMath.unpremultiply(outPixels, outPixels.length);
